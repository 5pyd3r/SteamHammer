    ...
    for (Building & b : _buildings)
    {
		...
		else if (!b.builderUnit->isConstructing())
        {
			if (!isBuildingPositionExplored(b))
            {
				...
				The::Instance().micro.Move(b.builderUnit, b.getCenter());
            }
            ...
            else
            {
				...
				b.buildCommandGiven = The::Instance().micro.Build(b.builderUnit, b.type, b.finalPosition);
...
void BuildingManager::cancelBuilding(Building & b)
{
	...
	else if (b.status == BuildingStatus::UnderConstruction)
	{
		...
		{
			The::Instance().micro.Cancel(b.buildingUnit);
			BuildingPlacer::Instance().freeTiles(b.finalPosition, b.type.tileWidth(), b.type.tileHeight());
		}
		toRemove.push_back(b);
		undoBuildings(toRemove);
...
BWAPI::TilePosition BuildingManager::getBuildingLocation(const Building & b)
{
	...
	if (b.type.isResourceDepot())
	{
		...
		if (b.macroLocation == MacroLocation::Front &&
			front.isValid() &&
			!The::Instance().groundAttacks.inRange(b.type, front))
		{
		...
		if (b.macroLocation == MacroLocation::Natural &&
			natural &&
			!The::Instance().groundAttacks.inRange(b.type, natural->getTilePosition()))
		{
			return natural->getTilePosition();
		}
