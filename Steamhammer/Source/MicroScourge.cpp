...
void MicroScourge::assignTargets(const BWAPI::Unitset & scourge, const BWAPI::Unitset & targets)
{
	...
	for (BWAPI::Unit target : targets)
	{
		if (target->isVisible() &&
            ...
            !The::Instance().airAttacks.inRange(target->getTilePosition()) &&	// skip defended targets
			target->isDetected() &&
			!target->isInvincible())
		{
	...
	for (const auto scourgeUnit : scourge)
	{
		...
		if (target)
		{
			...
			The::Instance().micro.CatchAndAttackUnit(scourgeUnit, target);
		}
		else
		{
			...
			{
				The::Instance().micro.MoveNear(scourgeUnit, order.getPosition());
                if (Config::Debug::DrawUnitTargetInfo)
                {
