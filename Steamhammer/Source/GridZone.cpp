...
// - To get the zone id, use The::Instance().zone->at(). It's often all you need.
// - To get a zone pointer, or null for zone 0, use The::Instance().zone->ptr(). The most similar method to BWTA::Region.
...
void GridZone::sanityCheck()
{
	...
	for (Zone * zone : zones)
	{
		if (zone->isValid())
		{
			...
			for (const BWAPI::TilePosition & tile : zone->tiles())
			{
				UAB_ASSERT(tile.isValid() && The::Instance().tileRoom.at(tile) > 0, "bad tile in zone");
			}
...
void GridZone::initialize()
{
	...
	for (int x = 0; x < width; ++x)
	{
		for (int y = 0; y < height; ++y)
		{
			...
			if (grid[x][y] == 0 && The::Instance().tileRoom.at(xy) >= minRoom)
			{
				// 3. Fill in the next zone.
				grid[x][y] = zoneID;
				zones.push_back(new Zone(zoneID));
				Zone * zone(zones.back());
				zone->_state = The::Instance().tileRoom.at(xy) <= chokeWidth ? ZoneState::Choke : ZoneState::Normal;
				...
				for (size_t fringeIndex = 0; fringeIndex < fringe.size(); ++fringeIndex)
				{
					...
					for (size_t a = 0; a < LegalActions; ++a)
					{
						...
						if (nextTile.isValid())
						{
							...
							else if (The::Instance().tileRoom.at(nextTile) >= minRoom &&
								(zone->isChoke()
									? The::Instance().tileRoom.at(nextTile) <= chokeWidth
									: The::Instance().tileRoom.at(nextTile) > chokeWidth && GroundHeight(nextTile) == zone->_groundHeight))
							{
