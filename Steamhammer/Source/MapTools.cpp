...
buildingLoopExit:
        ...
        if (The::Instance().groundAttacks.inRange(player->getRace().getResourceDepot(), topLeft))
		{
			continue;
		}
		...
		if (distanceFromUs < 0)
        {
            if (The::Instance().partitions.id(topLeft) == The::Instance().partitions.id(myBasePosition))
			{
		...
		if (enemyBase) {
			...
			if (distanceFromEnemy < 0)
			{
                ...
                if (The::Instance().partitions.id(topLeft) == The::Instance().partitions.id(enemyTile))
				{
