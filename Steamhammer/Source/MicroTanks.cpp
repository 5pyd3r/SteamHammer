	...
	for (const BWAPI::Unit tank : tanks)
	{
		if (order.isCombatOrder())
		{
			if (!tankTargets.empty())
			{
                ...
                {
					The::Instance().micro.Siege(tank);
				}
				else if (tank->canUnsiege() && shouldUnsiege)
                {
					The::Instance().micro.Unsiege(tank);
                }
				else if (target)
				{
					if (tank->isSieged())
					{
						The::Instance().micro.AttackUnit(tank, target);
					}
					else
					{
						The::Instance().micro.KiteTarget(tank, target);
					}
				}
 			}
			else
			{
				...
				if (tank->getDistance(order.getPosition()) > 100)
				{
                    ...
                    {
    					The::Instance().micro.Move(tank, order.getPosition());
                    }
				}
			}
		}
