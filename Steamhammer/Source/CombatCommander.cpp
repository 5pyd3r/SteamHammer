...
void CombatCommander::update(const BWAPI::Unitset & combatUnits)
{
	...
	if (BWAPI::Broodwar->getFrameCount() % 20 == 1)
	{
		doLarvaTrick();
	}
...
void CombatCommander::chooseScourgeTarget(const Squad & sourgeSquad)
{
	...
	for (const auto & kv : InformationManager::Instance().getUnitData(BWAPI::Broodwar->enemy()).getUnits())
	{
		...
		if (!ui.type.isFlyer() ||
            ...
            The::Instance().airAttacks.inRange(BWAPI::TilePosition(ui.lastPosition)))
		{
			continue;
		}
...
void CombatCommander::updateAttackSquads()
{
	...
	_carrierCount = UnitUtil::GetCompletedUnitCount(BWAPI::UnitTypes::Protoss_Carrier);
...
void CombatCommander::updateScoutDefenseSquad() 
{
	...
	const Zone * myZone = The::Instance().zone.ptr(Bases::Instance().myStartingBase()->getTilePosition());
    if (myZone)
    {
        return;
    }
    ...
    {
        if (myZone == The::Instance().zone.ptr(unit->getTilePosition()))
        {
...
void CombatCommander::updateBaseDefenseSquads() 
{
	...
	for (Base * base : Bases::Instance().getBases())
	{
		...
		const Zone * zone = The::Instance().zone.ptr(base->getTilePosition());
		...
		for (BWAPI::Unit unit : BWAPI::Broodwar->enemy()->getUnits())
		{
			...
			if (dist < defenseRadius ||
				dist < defenseRadius + 300 && zone == The::Instance().zone.ptr(unit->getTilePosition()))
			{
		...
		for (const auto unit : BWAPI::Broodwar->self()->getUnits())
		{
			if ((unit->getType() == BWAPI::UnitTypes::Terran_Missile_Turret ||
				...
				zone == The::Instance().zone.ptr(unit->getTilePosition()))
			{
		...
		if (bunker->getType() == BWAPI::UnitTypes::Terran_Bunker)
		{
			...
			if (enemyIsNear)
			{
				...
				if (bunker->getSpaceRemaining() > 0)
				{
					...
					if (marine)
					{
						The::Instance().micro.Load(bunker, marine);
					}
				}
			}
			else
			{
				The::Instance().micro.UnloadAll(bunker);
			}
		}
	}
}
...
void CombatCommander::doComsatScan()
{
	...
	for (const auto unit : BWAPI::Broodwar->enemy()->getUnits())
	{
			...
			unit->getPosition().isValid())
		{
			...
			(void) The::Instance().micro.Scan(unit->getPosition());
...
{
	if (BWAPI::Broodwar->getFrameCount() < 5040 &&
		...
		UnitUtil::GetCompletedUnitCount(BWAPI::UnitTypes::Zerg_Spawning_Pool) == 0)
	{
		for (Base * base : Bases::Instance().getBases())
		{
			...
			{
				The::Instance().micro.LarvaTrick(Bases::Instance().myMainBase()->getDepot()->getLarva());
			}
		}
	}
}
	...
	for (const auto unit : BWAPI::Broodwar->self()->getUnits())
	{
			...
			))
		{
                ...
                type == BWAPI::UnitTypes::Zerg_Sunken_Colony && unit->getHitPoints() < 130 && unit->getRemainingBuildTime() < 24)
            {
                ...
                {
                    The::Instance().micro.Cancel(unit);
                }
            }
		}
	}
		...
		*/
		...
		for (Base * base : Bases::Instance().getBases())
		{
			if (base->getBlockers().size() > 0 &&
				...
				squadPartition == The::Instance().partitions.id(base->getPosition()))
			{
...
BWAPI::Position CombatCommander::getAttackLocation(const Squad * squad)
{
	...
	//	? squad->mapPartition()
	//	: The::Instance().partitions.id(Bases::Instance().myStartingBase()->getTilePosition());
	const int squadPartition = The::Instance().partitions.id(Bases::Instance().myStartingBase()->getTilePosition());
	...
	if (canAttackGround)
	{
		...
		for (Base * base : Bases::Instance().getBases())
		{
			if (base->getOwner() == BWAPI::Broodwar->enemy())
			{
				...
				if (hasGround && squadPartition != The::Instance().partitions.id(base->getTilePosition()))
				{
					continue;
				}
				...
				// Mildly prefer to attack a base other than the enemy main. It's a simple heuristic.
				if (base == Bases::Instance().enemyStart())
				{
					score -= 1;
				}
	...
	if (canAttackGround || BWAPI::Broodwar->enemy()->getRace() == BWAPI::Races::Terran)
	{
		for (const auto & kv : InformationManager::Instance().getUnitInfo(BWAPI::Broodwar->enemy()))
		{
			...
			if (ui.type.isBuilding() &&
				...
				(ui.type.isRefinery() || squadPartition == The::Instance().partitions.id(ui.lastPosition)))
				// (!hasGround || (!ui.type.isRefinery() && squadPartition == The::Instance().partitions.id(ui.lastPosition))))
			{
				if (ui.lifted)
				{
	...
	for (const auto unit : BWAPI::Broodwar->enemy()->getUnits())
	{
		...
		if (hasGround &&
			squadPartition != The::Instance().partitions.id(unit->getPosition()) &&
...
BWAPI::Position CombatCommander::getDropLocation(const Squad & squad)
{
	...
	*/
	...
	Base * enemyMain = Bases::Instance().enemyStart();
	// if (enemyMain && enemyMain->getOwner() == BWAPI::Broodwar->enemy())
	if (enemyMain)
	{
	...
	for (Base * base : Bases::Instance().getBases())
	{
		if (base->getOwner() == BWAPI::Broodwar->enemy())
		{
			int score = 0;     // the final score will be 0 or negative
			std::vector<UnitInfo> enemies;
			InformationManager::Instance().getNearbyForce(enemies, base->getCenter(), BWAPI::Broodwar->enemy(), 600);
			for (const auto & enemy : enemies)
			{
...
int CombatCommander::numZerglingsInOurBase() const
{
	...
	BWAPI::Position myBasePosition(Bases::Instance().myStartingBase()->getPosition());
