...
void MicroDetectors::go(const BWAPI::Unitset & squadUnits)
{
	...
	for (const BWAPI::Unit detectorUnit : detectorUnits)
	{
		if (squadSize == 1)
		{
			...
			The::Instance().micro.MoveNear(detectorUnit, order.getPosition());
			return;
		}
            ...
            detectorUnit->getDistance(nearestCloaked) > 9 * 32)      // detection range is 11 tiles
        {
            ...
            !The::Instance().airAttacks.at(unitClosestToEnemy->getTilePosition()))
		{
		...
		The::Instance().micro.MoveNear(detectorUnit, destination);
	}
}
