...
void ScoutManager::moveGroundScout()
{
	...
	if (_workerScoutTarget.isValid())
	{
		...
		The::Instance().micro.Move(_workerScout, BWAPI::Position(_workerScoutTarget));
	}
	else
	{
		...
		if (scoutInRangeOfenemy)
		{
			...
			else
			{
					...
					_workerScout->getHitPoints() + _workerScout->getShields() > 20)
				{
					...
					The::Instance().micro.CatchAndAttackUnit(_workerScout, closestWorker);
				}
...
void ScoutManager::followGroundPath()
{
	...
	if (The::Instance().zone.at(enemyBase->getTilePosition()) != The::Instance().zone.at(_workerScout->getTilePosition()))
	{
		// We're not there yet. Go there.
		The::Instance().micro.Move(_workerScout, enemyBase->getCenter());
		return;
	}
	...
	if (_nextDestination.isValid() && _workerScout->getDistance(_nextDestination) > 96)
	{
		...
		The::Instance().micro.Move(_workerScout, _nextDestination);
		return;
	}
	...
	BWAPI::Position destination = MapGrid::Instance().getLeastExplored(
		true,
		The::Instance().partitions.id(enemyBase->getTilePosition()),
		The::Instance().zone.at(enemyBase->getTilePosition()));
...
void ScoutManager::moveAirScout()
{
	...
	if (enemyBase)
	{
		...
		if (!_overlordAtEnemyBase)
		{
			...
			The::Instance().micro.Move(_overlordScout, enemyBase->getCenter());
	...
	else
	{
		...
		{
			The::Instance().micro.Move(_overlordScout, BWAPI::Position(_overlordScoutTarget));
		}
	}
}
...
bool ScoutManager::gasSteal()
{
	...
	else if (_enemyGeyser->isVisible() && _workerScout->getDistance(_enemyGeyser) < 300)
	{
		...
		if (!ProductionManager::Instance().isGasStealInQueue())
		{
			...
			The::Instance().micro.Move(_workerScout, _enemyGeyser->getInitialPosition());
		}
		_gasStealStatus = "Stealing gas";
	}
	else
