...
// This depends on The::Instance().partitions already being initialized.
void GridInset::initialize()
{
	...
	{
		if (The::Instance().partitions.walkable(x, 0))
		{
		...
		if (The::Instance().partitions.walkable(x, height-1))
		{
	...
	{
		if (The::Instance().partitions.walkable(0, y))
		{
		...
		if (The::Instance().partitions.walkable(width-1, y))
		{
	...
	for (int x = 1; x < width-1; ++x)
	{
		...
		{
			if (The::Instance().partitions.walkable(x, y))
			{
				if (!The::Instance().partitions.walkable(x + 1, y) ||
					!The::Instance().partitions.walkable(x - 1, y) ||
					!The::Instance().partitions.walkable(x, y + 1) ||
					!The::Instance().partitions.walkable(x, y - 1))
				{
...
BWAPI::Position GridInset::find(const BWAPI::Position & start, int inset)
{
	...
	const int zoneID = The::Instance().zone.at(start);
	...
	while (at(here) != inset)
	{
		...
		for (size_t a = 0; a < LegalActions; ++a)
		{
			...
			if (next.isValid() && at(next) > 0 && The::Instance().zone.at(next) == zoneID &&
