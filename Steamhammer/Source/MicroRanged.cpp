...
void MicroRanged::assignTargets(const BWAPI::Unitset & rangedUnits, const BWAPI::Unitset & targets)
{
    ...
    for (const auto rangedUnit : rangedUnits)
	{
		...
		if (The::Instance().micro.fleeDT(rangedUnit))
		{
		...
		if (rangedUnit->isIrradiated() && rangedUnit->getType().isOrganic())
		{
			if (rangedUnit->isFlying())
			{
				...
				{
					The::Instance().micro.AttackMove(rangedUnit, order.getPosition());
				}
				else
				{
					The::Instance().micro.Move(rangedUnit, order.getPosition());
				}
				continue;
			}
			else if (rangedUnit->canBurrow())
			{
				The::Instance().micro.Burrow(rangedUnit);
				continue;
			}
		}
		...
		if (stayHomeUntilReady(rangedUnit))
		{
			...
			The::Instance().micro.AttackMove(rangedUnit, fleeTo);
			continue;
		}
		...
		if (order.isCombatOrder())
        {
			...
			if (target)
			{
				...
				{
					The::Instance().micro.KiteTarget(rangedUnit, target);
				}
				else
				{
					The::Instance().micro.CatchAndAttackUnit(rangedUnit, target);
				}
			}
			else
			{
...
BWAPI::Unit MicroRanged::getTarget(BWAPI::Unit rangedUnit, const BWAPI::Unitset & targets, bool underThreat)
{
	...
	for (const auto target : targets)
	{
        ...
        if (!underThreat)
        {
            ...
            if (rangedUnit->isFlying() ? !The::Instance().airAttacks.inRange(target) : !The::Instance().groundAttacks.inRange(target))
            {
                score += 4 * 32;
            }
        }
