...
void MicroOverlords::goToSpore(BWAPI::Unit overlord) const
{
	...
	if (spore)
	{
		...
		{
			The::Instance().micro.Stop(overlord);
		}
		else
		{
			The::Instance().micro.Move(overlord, spore->getPosition());
		}
	}
	else
	{
...
void MicroOverlords::update()
{
	...
	else
	{
		...
		for (std::pair<Base *, BWAPI::Unit> assignment : baseAssignments)
		{
			The::Instance().micro.Move(assignment.second, assignment.first->getPosition());
		}
