...
bool Bases::checkIslandMap() const
{
	...
	int ourPartition = The::Instance().partitions.id(startingBase->getTilePosition());

	for (BWAPI::TilePosition pos : BWAPI::Broodwar->getStartLocations())
	{
		// Is any other start position reachable from here by ground?
		if (pos != startingBase->getTilePosition() && ourPartition == The::Instance().partitions.id(pos))
		{
			return false;
		}
	}
...
void Bases::updateEnemyStart()
{
	...
	for (Base * start : startingBases)
	{
		...
		{
			int startZone = The::Instance().zone.at(start->getTilePosition());
			if (startZone > 0)
			{
				...
				for (BWAPI::Unit enemy : BWAPI::Broodwar->enemy()->getUnits())
				{
					if (enemy->getType().isBuilding() &&
						startZone == The::Instance().zone.at(enemy->getTilePosition()) &&
						!enemy->isLifted())
					{
						if (Config::Debug::DrawScoutInfo)
						{
...
void Bases::checkBuildingPosition(const BWAPI::TilePosition & desired, const BWAPI::TilePosition & actual)
{
	...
	if (The::Instance().zone.at(desired) == The::Instance().zone.at(mainBase->getTilePosition()))
	{
		// We tried to place in the main base, so let's keep checking.
		if (!actual.isValid() || The::Instance().zone.at(actual) != The::Instance().zone.at(desired))
		{
...
{
	//The::Instance().partitions.drawWalkable();
	//The::Instance().partitions.drawPartition(
	//	The::Instance().partitions.id(InformationManager::Instance().getMyMainBaseLocation()->getPosition()),
	//	BWAPI::Colors::Teal);

	//The::Instance().inset.draw();
	//The::Instance().vWalkRoom.draw();
	//The::Instance().tileRoom.draw();
	//The::Instance().zone.draw();
...
{
	return The::Instance().partitions.id(pos) == The::Instance().partitions.id(startingBase->getTilePosition());
}
...
{
	return The::Instance().partitions.id(tile) == The::Instance().partitions.id(startingBase->getTilePosition());
}
...
{
	int mainZone = The::Instance().zone.at(myStartingBase()->getTilePosition());
	int naturalZone = myNaturalBase() ? The::Instance().zone.at(myNaturalBase()->getTilePosition()) : 0;
	...
	for (const auto & kv : InformationManager::Instance().getUnitData(BWAPI::Broodwar->enemy()).getUnits())
	{
		...
		{
			int zone = The::Instance().zone.at(ui.lastPosition);
			if (zone > 0)
			{
