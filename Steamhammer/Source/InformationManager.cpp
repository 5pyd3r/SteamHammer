...
void InformationManager::initializeRegionInformation()
{
	...
	updateOccupiedRegions(The::Instance().zone.ptr(Bases::Instance().myStartingBase()->getTilePosition()), _self);
}
...
void InformationManager::updateBaseLocationInfo() 
{
	...
	for (const auto & kv : _unitData[_enemy].getUnits())
	{
		...
		{
			updateOccupiedRegions(The::Instance().zone.ptr(BWAPI::TilePosition(ui.lastPosition)), _enemy);
		}
	}
	...
	for (const BWAPI::Unit unit : _self->getUnits())
	{
		...
		{
			updateOccupiedRegions(The::Instance().zone.ptr(unit->getTilePosition()), _self);
		}
	}
}
...
bool InformationManager::isEnemyBuildingInRegion(const Zone * zone) 
{
	...
	for (const auto & kv : _unitData[_enemy].getUnits())
	{
		...
		if (ui.type.isBuilding() && !ui.goneFromLastPosition)
		{
			if (zone->id() == The::Instance().zone.at(ui.lastPosition)) 
			{
				return true;
			}
		}
