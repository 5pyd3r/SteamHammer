...
void MicroMutas::reinforce(const BWAPI::Unitset & stragglers, const BWAPI::Position & center)
{
	...
	{
		The::Instance().micro.Move(muta, center);
...
void MicroMutas::attackAssignedTargets(const BWAPI::Position & center)
{
	...
	if (assignments.empty())
	{
		...
		for (BWAPI::Unit muta : getUnits())
		{
			...
			{
				The::Instance().micro.Move(muta, order.getPosition());
			}
			else
			{
				The::Instance().micro.Move(muta, center);
	...
	else
	{
		...
		for (std::pair<BWAPI::Unit, BWAPI::Unit> assignment : assignments)
		{
			...
			{
				The::Instance().micro.MutaDanceTarget(muta, target);
			}
			else
			{
				The::Instance().micro.CatchAndAttackUnit(muta, target);
			}
		}
	}
}
...
void MicroMutas::executeMicro(const BWAPI::Unitset & targets, const UnitCluster & cluster)
{
	...
	for (BWAPI::Unit muta : getUnits())
	{
		if (muta->isIrradiated())
		{
			...
			{
				The::Instance().micro.Move(muta, getFleePosition(muta, center, targets));
			}
			else
			{
				The::Instance().micro.AttackMove(muta, getFleePosition(muta, center, targets));
			}
		}
