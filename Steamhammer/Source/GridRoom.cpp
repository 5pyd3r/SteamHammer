...
// This depends on The::Instance().inset already being initialized.
...
looptop:
		while (y < height)
		{
			int value = The::Instance().inset.at(x, y);
			if (value <= 0)
			{
				++y;
			}
			else
			{
				...
				for (; y < height; ++y)
				{
					...
					if (y == height - 1)
					{
						...
						value = The::Instance().inset.at(x, y);
						foundMax = true;
					}
					else
					{
						const int newValue = The::Instance().inset.at(x, y + 1);
						if (newValue > value)
						{
							value = The::Instance().inset.at(x, y + 1);
						}
					...
					if (foundMax)
					{
						...
						while (y < height - 1)
						{
							if (The::Instance().inset.at(x, y + 1) <= 0 || The::Instance().inset.at(x, y + 1) > The::Instance().inset.at(x, y))
							{
								break;
							}
							++y;
