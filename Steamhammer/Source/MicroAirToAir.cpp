...
void MicroAirToAir::assignTargets(const BWAPI::Unitset & airUnits, const BWAPI::Unitset & targets)
{
    ...
    for (const auto airUnit : airUnits)
	{
		...
		if (airUnit->isIrradiated() && airUnit->getType().getRace() == BWAPI::Races::Zerg)
		{
			...
			{
				The::Instance().micro.AttackMove(airUnit, order.getPosition());
			}
			else
			{
				The::Instance().micro.Move(airUnit, order.getPosition());
			}
			continue;
		}
		...
		if (order.isCombatOrder())
        {
			...
			if (target)
			{
				...
				The::Instance().micro.CatchAndAttackUnit(airUnit, target);
			}
			else
			{
